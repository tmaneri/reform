...
function useReform(fields) {
  const defaultValues = Object.keys(fields).reduce((carry, key) => {
    return __spreadProps(__spreadValues({}, carry), {
      [key]: fields[key].value
    });
  }, {});
  const data = reactive(__spreadValues({}, defaultValues));
  const pristine = reactive(__spreadValues({}, defaultValues));
  const errors = reactive(mapValues(fields, () => null));
  ...
  const changes = computed(() => {
    return Object.keys(fields).reduce((changes2, name) => {
      if (!isEqual(pristine[name], data[name])) {
        changes2[name] = data[name];
      }
      return changes2;
    }, {});
  });
  const validators = Object.keys(fields).reduce((carry, key) => {
